  ...
  const playerSchema = new mongoose.Schema({
    ...
    // Phone + OTP
    phone: {
      number: { type: String, default: undefined },  // will be unique across players
  ...
  playerSchema.index(
    ...
    {
      unique: true,
      partialFilterExpression: { 'phone.number': { $exists: true, $ne: '' } }
    }
  );
  ...
  app.patch('/api/journey/:id', async (req, res) => {
    try {
      ...
      if (typeof req.body.phoneNumber === 'string') {
        ...
        if (n) {                // ⟵ only set when truthy
          updates['phone.number'] = n;
          steps.phone = true;
        }
      }
  ...
  app.patch('/api/player/:sessionId', async (req, res) => {
    try {
      ...
      if (typeof body.phone === 'string') {
        ...
        if (normalized) {       // ⟵ only set when truthy
          update['phone.number'] = normalized;
          update['steps.phone'] = true;
        }
      }
