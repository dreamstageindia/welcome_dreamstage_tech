...
(function () {
  ...
  /* Hide by default to avoid flicker before role resolve */
  hideWhatsAppButton();
  ...
  async function resolveRoleAndRoute(joinOrder) {
    ...
    try {
        ...
        if (r.ok) {
          ...
          if (j && j.playerId) {
            ...
            if (g.ok) {
              ...
              // If artist -> hide WA button and redirect to EPK after 10 seconds
              if (role === 'artist') {
                hideWhatsAppButton();
            ...
            // Not artist or no role: show button and set up WhatsApp auto-open
      ...
      // If anything fails, fallback to WA auto-open for non-artist flow
    }
    scheduleWhatsAppAutoOpen();
  }
